...
def build_layout_skeleton(card_type: str, card_data: dict, grade: str = '') -> str:
    ...
    if _is_xiaoxue and card_type == '情景对话卡':
        ...
        lines.append('')
        # v10.27 P1a: 角色-气泡方向绑定 + 单气泡字数限制
        lines.append('  💬 SPEAKER-BUBBLE ALIGNMENT (v10.27 — 角色对话对齐):')
        lines.append('    • ⚠️ LEFT character\'s bubbles must have tail pointing LEFT (toward the left character)')
        lines.append('    • ⚠️ RIGHT character\'s bubbles must have tail pointing RIGHT (toward the right character)')
        lines.append('    • ⚠️ Each speech bubble MUST contain ≤ 6 English words — if longer, SPLIT into 2 bubbles or SHORTEN the sentence')
        lines.append('    • Alternate bubble positions: Character A\'s bubble on left side, Character B\'s bubble on right side')
        lines.append('    • Bubble vertical order must match dialogue flow: first exchange at top, last at bottom')
        lines.append('    • NEVER place two bubbles from the SAME character overlapping or adjacent without clear speaker indicator')
        lines.append('')
        ...
        lines.append('    □ Each bubble has ≤ 6 English words')
        lines.append('    □ Bubble tails clearly point to the correct speaker')
        lines.append('')
