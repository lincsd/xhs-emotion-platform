...
QUALITY_RUBRICS = {
    ...
    '语法辨析卡': {
        'dimensions': {
            '语法准确性': {'weight': 25, 'desc': '语法规则/引导词用法100%正确'},
            '中英对照':   {'weight': 20, 'desc': '中英双语对照是否清晰准确'},
            '例句质量':   {'weight': 15, 'desc': '例句是否地道、典型、无语法错'},
            '对比清晰度': {'weight': 15, 'desc': '不同用法的对比是否一目了然'},
            '视觉美感':   {'weight': 10, 'desc': '配色/排版/图标是否美观'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题(如"句型卡：XXX")直接0分！底部暖色口诀条不算Banner'},
        },
        'must_have': ['语法规则', '中英双语例句', '用法对比'],
        'red_flags': ['英语语法错误', '翻译不准确', '引导词混淆', '顶部深色标题栏Banner'],
    },
    '单词卡': {
        'dimensions': {
            '知识准确性': {'weight': 25, 'desc': '单词拼写/音标/释义是否正确'},
            '例句质量':   {'weight': 15, 'desc': '例句是否地道、难度匹配'},
            '记忆技巧':   {'weight': 15, 'desc': '联想/词根/对比是否有效'},
            '发音标注':   {'weight': 15, 'desc': '音标/自然拼读是否标注'},
            '视觉美感':   {'weight': 15, 'desc': '配色/排版是否美观'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题(如"词汇卡：XXX")直接0分！底部暖色口诀条不算Banner'},
        },
        'must_have': ['单词', '释义', '例句'],
        'red_flags': ['单词拼写错误', '释义不准确', '音标错误', '顶部深色标题栏Banner'],
    },
    # ── 英语细分类（v10.31+: 完整覆盖所有英语卡片类型）──
    '词汇卡': {
        'dimensions': {
            '知识准确性': {'weight': 25, 'desc': '单词拼写/释义/音标是否正确'},
            '例句质量':   {'weight': 15, 'desc': '例句是否完整、地道、难度匹配'},
            '记忆技巧':   {'weight': 15, 'desc': '联想/词根/口诀是否有效'},
            '视觉美感':   {'weight': 15, 'desc': '配色/排版/图标是否美观有吸引力'},
            '实用价值':   {'weight': 15, 'desc': '词汇是否高频常考、有实用性'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！'},
        },
        'must_have': ['单词', '释义', '例句'],
        'red_flags': ['单词拼写错误', '释义不准确', '顶部深色标题栏Banner'],
    },
    '句型卡': {
        'dimensions': {
            '句型准确性': {'weight': 25, 'desc': '句型结构/用法是否100%正确'},
            '例句质量':   {'weight': 20, 'desc': '例句是否典型、完整(≥6词)、有变化'},
            '教学清晰度': {'weight': 15, 'desc': '句型展示是否一目了然、有替换演示'},
            '视觉美感':   {'weight': 10, 'desc': '配色/排版/图标是否美观'},
            '实用价值':   {'weight': 15, 'desc': '句型是否常考高频、有实用性'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！'},
        },
        'must_have': ['句型结构', '中英例句', '用法说明'],
        'red_flags': ['句型语法错误', '例句不完整', '顶部深色标题栏Banner'],
    },
    '语法卡': {
        'dimensions': {
            '语法准确性': {'weight': 25, 'desc': '语法规则/用法是否100%正确'},
            '中英对照':   {'weight': 20, 'desc': '中英双语解说是否清晰准确'},
            '例句质量':   {'weight': 15, 'desc': '例句是否地道、典型、无语法错'},
            '教学清晰度': {'weight': 10, 'desc': '规则呈现是否一步步清晰可懂'},
            '视觉美感':   {'weight': 15, 'desc': '配色/排版/图标是否美观'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！'},
        },
        'must_have': ['语法规则', '中英双语例句'],
        'red_flags': ['英语语法错误', '翻译不准确', '顶部深色标题栏Banner'],
    },
    '自然拼读卡': {
        'dimensions': {
            '拼读准确性': {'weight': 25, 'desc': '字母组合/发音规则是否正确'},
            '例词质量':   {'weight': 20, 'desc': '范例单词是否典型、发音一致'},
            '记忆技巧':   {'weight': 15, 'desc': '口诀/韵律是否朗朗上口'},
            '视觉美感':   {'weight': 15, 'desc': '配色/排版/图标是否美观有吸引力'},
            '教学趣味性': {'weight': 10, 'desc': '是否有趣味元素吸引低龄学生'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！'},
        },
        'must_have': ['拼读规则', '范例单词', '发音标注'],
        'red_flags': ['发音规则错误', '例词发音不符合规则', '顶部深色标题栏Banner'],
    },
    '情景对话卡': {
        'dimensions': {
            '对话真实性': {'weight': 25, 'desc': '对话是否自然真实、贴近生活场景'},
            '语言准确性': {'weight': 20, 'desc': '英文表达是否地道、无语法错'},
            '教学清晰度': {'weight': 15, 'desc': '角色分工/对话流程是否一目了然'},
            '视觉美感':   {'weight': 15, 'desc': '配色/排版/角色标识是否美观'},
            '实用价值':   {'weight': 10, 'desc': '对话场景是否常见高频、可模仿'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！'},
        },
        'must_have': ['对话文本', '角色标识', '场景说明'],
        'red_flags': ['英语表达错误', '对话不自然', '顶部深色标题栏Banner'],
    },
    '易混词卡': {
        'dimensions': {
            '辨析准确性': {'weight': 25, 'desc': '易混词区分方法是否准确有效'},
            '对比清晰度': {'weight': 20, 'desc': '对比排版是否一目了然(左右/上下PK)'},
            '例句质量':   {'weight': 15, 'desc': '例句是否能清晰体现区别'},
            '记忆技巧':   {'weight': 10, 'desc': '辨别口诀/技巧是否有效'},
            '视觉美感':   {'weight': 15, 'desc': '配色/排版是否美观醒目'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！'},
        },
        'must_have': ['易混词对', '区别说明', '对比例句'],
        'red_flags': ['词义解释错误', '区别说反了', '顶部深色标题栏Banner'],
    },
    '不规则动词卡': {
        'dimensions': {
            '知识准确性': {'weight': 25, 'desc': '动词三形态(原形/过去式/过去分词)100%正确'},
            '分组合理性': {'weight': 20, 'desc': '是否按变化规律分组(如AAA/ABB/ABC)'},
            '记忆技巧':   {'weight': 15, 'desc': '口诀/韵律/联想是否有效'},
            '教学清晰度': {'weight': 10, 'desc': '表格/时间线展示是否清晰'},
            '视觉美感':   {'weight': 15, 'desc': '配色/排版是否美观'},
            '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！'},
        },
        'must_have': ['动词三形态', '变化规律'],
        'red_flags': ['动词形态拼写错误', '分组归类错误', '顶部深色标题栏Banner'],
    },
    # ── 考卷类 ──
    '考卷真题卡': {
        'dimensions': {
...
_DEFAULT_RUBRIC = {
    'dimensions': {
        '知识准确性': {'weight': 20, 'desc': '内容是否准确无误'},
        '教学清晰度': {'weight': 25, 'desc': '信息是否清晰易懂'},
        '视觉美感':   {'weight': 15, 'desc': '配色/排版是否美观'},
        '可收藏感':   {'weight': 15, 'desc': '干货感/想截图保存'},
        '实用价值':   {'weight': 10, 'desc': '内容是否实用有价值'},
        '无标题栏':   {'weight': 15, 'desc': '⚠️卡片顶部是否没有深色标题栏/Banner？顶部有深色横幅+白色中文标题直接0分！底部暖色口诀条不算Banner'},
    },
    'must_have': [],
    'red_flags': ['顶部深色标题栏Banner'],
}
...
def build_typed_quality_prompt(card_type, subject, card_title=''):
    ...
    # v10.31+: 如果有「无标题栏」维度，追加视觉检测说明
    banner_extra = ''
    if '无标题栏' in rubric.get('dimensions', {}):
        banner_extra = """

⚠️ 关于「无标题栏」维度的专项视觉检测：
- 仔细观察卡片图片**顶部区域（上方约15%的高度）**
- 如果看到深色横幅/渐变条 + 上面有白色中文大字（如"句型卡：XXX""词汇卡""语法卡""自然拼读卡""情景对话卡""易混词卡"），这就是Banner/标题栏 → 此维度给0分！
- 如果顶部直接是白色/浅色内容区，没有深色横幅 → 此维度给12-15分
- 注意：底部暖色口诀条/Slogan条不算Banner！只看顶部！
- 这是最重要的视觉检查项，请务必仔细看！"""
