...
CARD_TYPE_STRATEGY_MAP = {
    ...
    "实验卡": {
        ...
        "secondary": "发现式",
        "fallback": "对比式",
        "rationale": "实验本身是递进过程：目的→操作→现象→结论",
        "content_focus": {
            "must_show": ["实验目的(1句)", "关键步骤(≤5步)", "现象描述(有色彩)", "结论"],
            "nice_to_have": ["对照组设置", "安全提示", "器材简图"],
            "avoid": ["步骤过多(>5)", "无现象描述", "缺少结论"]
        }
    },
    "公式推导卡": {
        "primary": "递进式",
        "secondary": "类比式",
        "fallback": "发现式",
        "rationale": "推导是逐步递进的逻辑链，每步有因果关系",
        "content_focus": {
            "must_show": ["出发点(已知条件)", "逐步推导(标注含义)", "最终公式", "适用条件"],
            "nice_to_have": ["各符号注释", "常见误用", "代入验证"],
            "avoid": ["跳步省略", "只有公式无推导", "无适用条件"]
        }
    },
    "过程流卡": {
        "primary": "递进式",
        "secondary": "对比式",
        "fallback": "类比式",
        "rationale": "过程是时间/空间上的递进流程",
        "content_focus": {
            "must_show": ["各阶段名称+色块", "阶段间箭头(输入/输出)", "核心方程/反应"],
            "nice_to_have": ["物质/能量流向", "阶段对比表", "记忆口诀"],
            "avoid": ["阶段过多(>6)", "无箭头连接", "遗漏关键物质"]
        }
    },
    "微观图解卡": {
        "primary": "发现式",
        "secondary": "类比式",
        "fallback": "对比式",
        "rationale": "从宏观到微观是发现过程，揭示本质",
        "content_focus": {
            "must_show": ["宏观现象", "微观粒子图解", "宏微对应关系", "本质总结"],
            "nice_to_have": ["放大镜视觉效果", "粒子运动方向", "类比生活场景"],
            "avoid": ["粒子过多(>15)", "无宏观对照", "结构过于复杂"]
        }
    },
    "图像解读卡": {
        "primary": "发现式",
        "secondary": "递进式",
        "fallback": "类比式",
        "rationale": "读图是从图中发现信息的过程",
        "content_focus": {
            "must_show": ["示例图像(标注坐标)", "读图方法(看轴/看点/看趋势)", "考法提示"],
            "nice_to_have": ["关键点标注", "易错点", "变式图对比"],
            "avoid": ["无坐标标注", "读图步骤>3", "图像模糊不清"]
        }
    },
    "模型卡": {
        "primary": "类比式",
        "secondary": "发现式",
        "fallback": "对比式",
        "rationale": "科学模型本身就是类比和简化的产物",
        "content_focus": {
            "must_show": ["模型示意图", "核心假设(标注)", "适用范围", "局限性"],
            "nice_to_have": ["模型演变历史", "与其他模型对比", "现实应用"],
            "avoid": ["过于复杂的3D图", "无适用范围", "混淆模型与现实"]
        }
    },
    "解题策略卡": {
        "primary": "递进式",
        "secondary": "挑战式",
        "fallback": "对比式",
        "rationale": "解题策略是按步骤递进的方法论",
        "content_focus": {
            "must_show": ["N步法(编号色块)", "每步要点", "检查清单", "口诀"],
            "nice_to_have": ["题型识别方法", "常忘步骤红标", "适用题型范围"],
            "avoid": ["步骤>5", "教具体题(教方法)", "检查项>3"]
        }
    },
    "知识网络卡": {
        "primary": "递进式",
        "secondary": "发现式",
        "fallback": "类比式",
        "rationale": "知识网络从核心概念逐层展开",
        "content_focus": {
            "must_show": ["核心节点(居中)", "分支连线(层级)", "核心公式/规律"],
            "nice_to_have": ["跨章节连接", "重要节点加粗", "记忆口诀"],
            "avoid": ["节点>12", "交叉线过密", "深度>3层"]
        }
    },
    "术语精准卡": {
        "primary": "对比式",
        "secondary": "递进式",
        "fallback": "挑战式",
        "rationale": "术语精准靠❌/✅对比纠正",
        "content_focus": {
            "must_show": ["错误表述(红)", "精准表述(绿)", "扣分原因", "记忆技巧"],
            "nice_to_have": ["高考真题示例", "同类易错词归类", "助记联想"],
            "avoid": ["对比>4组", "解释过长(>30字/条)", "堆砌不相关术语"]
        }
    }
}
